...
async function sendResponse(path: string,
  ...
  errCallback: () => void = () => console.error('error')) {
  const options = {
    hostname: '127.0.0.1',
    port: 3000,
  ...
  http.request(options, (res: any) => {
    res.on('data', (body: any) => {
      const decode = new TextDecoder("utf-8").decode(body);
      if (decode === 'error') {
        errCallback();
      }
      const fromJSON = JSON.parse(decode);
      callback(fromJSON);
    });
...
async function updateState(fieldWidth: number, fieldHeight: number) {
  ...
  sendResponse('get_state', {
    ...
    errBlock.innerHTML = 'GAME OVER!!!';
    errBlock.classList.add('game-over');
    document.body.append(errBlock);
  });
}
